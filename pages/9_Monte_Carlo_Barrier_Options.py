...
def monte_carlo_barrier_option_combined(S0, K, H, T, r, q, sigma, M, N, option_type, rebate=0,
    ...
    if use_control_variate:
        vanilla_analytic = black_scholes_call_price(S0, K, T, r, sigma) if is_call else black_scholes_put_price(S0, K, T, r, sigma)
        barrier_price_cv = barrier_price_mc - vanilla_mc + vanilla_analytic
    else:
        barrier_price_cv = barrier_price_mc
...
if st.sidebar.button("Calculate Option Price"):
    ...
    # -------------------------------------------------------------------------
    # Plot some of the +z paths in the preferred style
    # -------------------------------------------------------------------------
    num_paths_to_plot = min(500, int(N))
    time_grid = np.linspace(0, T, int(M) + 1)
    sample_paths = []  # We'll simulate one path per iteration
    sample_status = [] # True if active (knock-out: barrier never breached; knock-in: barrier breached)
    
    # For barrier options:
    # For knock-out: active if barrier is never breached.
    # For knock-in: active if barrier is breached.
    is_call = 'call' in option_type.lower()
    is_up = 'up' in option_type.lower()
    is_in = 'in' in option_type.lower()
    ...
    for i in range(num_paths_to_plot):
        ...
        S_val = S0
        breached = False
        for t in range(1, int(M) + 1):
            z = np.random.standard_normal()
            S_val = S_val * np.exp((r - q - 0.5 * sigma**2) * (T/int(M)) + sigma * np.sqrt(T/int(M)) * z)
            S_path.append(S_val)
            if not breached:
                if (is_up and S_val >= H) or ((not is_up) and S_val <= H):
                    breached = True
        sample_paths.append(S_path)
        # Define active status: for knock-in, active if barrier breached; for knock-out, active if never breached.
        sample_status.append(breached if is_in else (not breached))
    
    fig = go.Figure()
    for i in range(num_paths_to_plot):
        path = sample_paths[i]
        # Use RGBA color for transparency (alpha=0.5)
        color = "rgba(0,255,0,0.7)" if sample_status[i] else "rgba(255,0,0,0.7)"
        fig.add_trace(go.Scatter(
            x=time_grid, y=path,
            mode='lines',
            line=dict(color=color, width=1),
    ...
    # Add a dashed horizontal line representing the barrier level
    fig.add_trace(go.Scatter(
        x=[0, T], y=[H, H],
        mode='lines',
        line=dict(color='blue', dash='dash'),
